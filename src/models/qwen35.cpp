...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn_linear(
    ...
    static const bool direct_conv_state = [] {
        const char * value = std::getenv("LLAMA_CUDA_SSM_CONV_DIRECT_STATE");
        return value != nullptr && std::atoi(value) != 0;
    }();

    ggml_tensor * conv_output_proper;
    if (direct_conv_state && n_seqs == 1 && n_seq_tokens <= 32) {
        ggml_tensor * qkv_transposed = ggml_transpose(ctx0, qkv_mixed);
        conv_output_proper = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, conv_channels, n_seq_tokens, n_seqs);
        conv_output_proper->op = GGML_OP_SSM_CONV;
        conv_output_proper->src[0] = conv_states_all;
        conv_output_proper->src[1] = conv_kernel;
        conv_output_proper->src[2] = qkv_transposed;
        conv_output_proper->src[3] = inp->s_copy_main;
        const int32_t params[] = {
            (int32_t) inp->mctx->get_size(),
            (int32_t) inp->mctx->get_head(),
            (int32_t) cparams.n_rs_seq + 1,
            (int32_t) inp->mctx->get_rs_z(),
        };
        memcpy(conv_output_proper->op_params, params, sizeof(params));
    } else {
